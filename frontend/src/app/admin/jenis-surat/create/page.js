...
export default function CreateJenisSuratPage() {
  ...
  useEffect(() => {
    const fetchInit = async () => {
      ...
      try {
        const catRes = await apiFetch("/api/letters/categories");
        const catJson = await catRes.json();
        if (catJson.success) {
          setCategories(catJson.data);
          setForm((prev) => ({
            ...prev,
            letterCategoryId: catJson.data[0]?.id ?? "",
          }));
        } else {
          setErrorInitial("Gagal memuat kategori surat");
        }
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrorSave("");
    try {
      const res = await apiFetch(`/api/admin/letter-types`, {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-xl font-bold text-gray-900 mb-6">Tambah Jenis Surat</h2>
